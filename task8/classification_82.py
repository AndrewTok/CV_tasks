...
def train_classifier(gt, img_dir, fast_train = True):
    ...
    data_module = BirdsClassificationDataModule(img_dir, gt, batch_size=16, fraction=0.8, train_transform=default_transform, val_transform=test_transform)

    training_module = BirdsClassifierTrainingModule(fast_train)

    if not fast_train:
        trainer = pl.Trainer(accelerator='cuda', devices=1, max_epochs=300)
