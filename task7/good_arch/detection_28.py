...
class FacesPointsDetector(torch.nn.Module):
    # увеличил число нейронов в лин. слоях
    # padding = same
    # сохранение лучшей модели через torch.save
    do = 0
    def __init__(self):
        super().__init__()
        ...
        self.layers = nn.Sequential(
            ...
            # DetectorBlock(512, 512, 3, True,self.do),
            Flattener(),
            nn.LazyLinear(out_features=720),
            nn.BatchNorm1d(720),
            nn.ReLU(),
...
class FacesPointsTrainingModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if loss < self.best_val_loss:
            ...
            torch.save(self.model.state_dict(), 'test_save/best.chkpt')
