...
class FacesPointsDetector(torch.nn.Module):
    # set dropout to 0.1
    # поднял lr чтобы вначале обучалось
    # добавил свертки без макспула
    # обычный relu 
    # проверим как влияет хвост, потом проверить влияние сверток без макспула, потом проверить влияение сверток но с одинаковым ядром
    # 11 модель с аугментациями
    do = 0.1
    def __init__(self):
        super().__init__()

        self.layers = nn.Sequential(
            DetectorBlock(3, 64, 7, False, self.do),
            DetectorBlock(64, 64, 7, True, self.do),
            DetectorBlock(64, 256, 5, False, self.do),
            DetectorBlock(256, 256, 5, True, self.do),
