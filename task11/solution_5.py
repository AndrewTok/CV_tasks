...
#source: https://github.com/Datasciensyash/SeismicNet/blob/master/modules/losses.py
def jaccard_loss(logits, true, eps=1e-7):
    ...
    if num_classes == 1:
        true_1_hot = torch.eye(num_classes + 1, device='cuda')[true.squeeze(1).to(torch.int64)]
        ...
        true_1_hot = torch.eye(num_classes, device='cuda')[true.squeeze(1).to(torch.int64)]
        true_1_hot = true_1_hot.permute(0, 3, 1, 2).float()
...
def train_segmentation():
    ...
    instance_loss = jaccard_loss #torch.nn.BCEWithLogitsLoss()
